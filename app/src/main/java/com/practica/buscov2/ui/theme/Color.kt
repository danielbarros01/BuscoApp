package com.practica.buscov2.ui.theme

import androidx.compose.ui.graphics.Color

//Colors
val OrangePrincipal = Color(0xFFFF5722)
val GrayText = Color(0xFF3E3E3E )
val GrayField = Color(0xE6757575)
val GrayPlaceholder = Color(0xE6969696)
val GrayInput = Color(0xFFEBEBEB)
val BlueLink = Color(0xFF2F89FC)
val YellowStar = Color(0xFFFBBC05)
val YellowGold = Color(0xFF8F7800)
val ShadowColor = Color(0x80000000)
val GreenBusco = Color(0xFF34A853)
val RedBusco = Color(0xFFFF0000)